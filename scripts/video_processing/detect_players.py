...
# This should point to the parent directory of the 'models' directory
yolov5_path = os.path.join(os.path.dirname(__file__), '../../models/yolov5')
...
def load_model(weights_path='../../models/yolov5/yolov5s.pt', device=''):
    """
    Load the YOLOv5 model from weights, adjust device assignment.
    """
    device = select_device(device)  # Selects the appropriate device
    model = attempt_load(weights_path)  # Load the model without map_location
...
def detect_players(frame, model, device):
    ...
    img, _, _ = letterbox(img, 640, stride=32)
    ...
    # Extract bounding boxes for detected players
    for det in pred:
        if len(det):
